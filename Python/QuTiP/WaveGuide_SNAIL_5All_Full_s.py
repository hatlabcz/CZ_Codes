...
C3 = qt.tensor(C2, IDEN_E)
C  = qt.tensor(C3, IDEN_S)
C_DAG = C.dag()

D0 = qt.tensor(IDEN_A,IDEN_B)
D1 = qt.tensor(D0, IDEN_C)
D2 = qt.tensor(D1, DES_D)
D3 = qt.tensor(D2, IDEN_E)
D  = qt.tensor(D3, IDEN_S)
D_DAG = D.dag()

E0 = qt.tensor(IDEN_A,IDEN_B)
E1 = qt.tensor(E0, IDEN_C)
E2 = qt.tensor(E1, IDEN_D)
E3 = qt.tensor(E2, DES_E)
E  = qt.tensor(E3, IDEN_S)
E_DAG = E.dag()

S0 = qt.tensor(IDEN_A,IDEN_B)
S1 = qt.tensor(S0, IDEN_C)
S2 = qt.tensor(S1, IDEN_D)
S3 = qt.tensor(S2, IDEN_E)
S  = qt.tensor(S3, DES_S)
S_DAG = S.dag()
...
I3 = qt.tensor(I2, IDEN_E)
I  = qt.tensor(I3, IDEN_S)
I_DAG = I.dag()
...
def pump5(t, args):
    return pump_amp * (np.exp(-1j * OP * t) \
           ...
           pump_amp * (np.exp(-1j * OP3 * t) \
           + np.exp(1j * OP3 * t))+\
           pump_amp * (np.exp(-1j * OP4 * t) \
           + np.exp(1j * OP4 * t))               
#======Time-Depedent Parameters=========

#=============Hamiltonians=============
HAMIL_TI = OA * A_DAG * A + OB * B_DAG * B + OC * C_DAG * C + OD * D_DAG * D + OE * E_DAG * E + OS * S_DAG * S +\
...
PSI2 = qt.tensor(PSI1, qt.fock(ND, 1))
PSI3 = qt.tensor(PSI2, qt.fock(NE, 0))
PSI  = qt.tensor(PSI3, qt.fock(NS, 0))
