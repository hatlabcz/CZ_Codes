...
def hfss_simulation():
    ...
    def func():
        ...
        chi_qq = -ec / (2 * pi * hbar)
        chi_cc = -ec / (2 * pi * hbar) * cavity_mode[3]**2/qubit_mode[3]**2
        chi_qc = -2.0 * math.sqrt(chi_qq * chi_cc)
        coupling = math.sqrt(chi_qc/(4.0 * chi_qq) * abs(xdata[qubit_mode[0]]-xdata[cavity_mode[0]])**2)
        ...
        entry_set[3].insert(0,str(xdata[cavity_mode[0]]))
        entry_set[4].insert(0,str(cavity_mode[2] * 1e9))
        entry_set[5].insert(0,str(chi_cc * 1e-6))
        entry_set[6].insert(0,str(chi_qc * 1e-6))
        entry_set[7].insert(0,str(coupling * 1e3))
        entry_set[8].insert(0,'Done')
        

    tk.Button(temp, text = 'Load file', command = func).grid(column = 0, row = 3)
